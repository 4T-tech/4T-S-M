...
/* 引脚操作宏定义 */
#define HX711_DOUT(x)       HAL_GPIO_WritePin(HX711_DOUT_GPIO_PORT, HX711_DOUT_GPIO_PIN, (GPIO_PinState)x)
#define HX711_SCK(x)        HAL_GPIO_WritePin(HX711_SCK_GPIO_PORT, HX711_SCK_GPIO_PIN, (GPIO_PinState)x)
...
static unsigned long hx711_readCount(void)
{
    ...
    for(uint8_t i = 0; i < 24; i++) {
        ...
        delay_us(1);
        HX711_SCK(0);
        count |= (HX711_DOUT_STATE ? 1UL : 0UL) << (23 - i); /* 修正位操作 */
        delay_us(1);
    }
    
    /* 用于设置输出增益和输出通道 */
    for(uint8_t j = 0; j < hx711.gain; j++) {
        HX711_SCK(1);
        delay_us(1);
        HX711_SCK(0);
        delay_us(1);
    ...
    return (count ^ 0x800000); /* 将补码转换为原码 */
}
...
{
    /* 启用DWT（Data Watchpoint and Trace）用于精确延时 */
